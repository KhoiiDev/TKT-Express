...
const controller = {
    ...
    postCreate: async (req, res) => {
        try {
            const { nameInput, positionInput, phoneInput, emailInput, workplace } = req.body;

            if (!req.file && workplace.trim() === '' && nameInput.trim() === '' && positionInput.trim() === '' && emailInput.trim() === '' && phoneInput.trim() === '') {
                res.status(400).send({
                    errorData: true,
                    reaction: false,
    ...
    postUpdate: async (req, res) => {
        try {
            let { nameInput, positionInput, phoneInput, filedName, id, emailInput, workplace } = req.body;

            if (nameInput.trim() === '' && positionInput.trim() === '' && phoneInput.trim() === '' && emailInput.trim() === '') {
                res.status(400).send({
                    errorData: true,
                    reaction: false,
                    message: 'Error post data',
    ...
    postDelete: async (req, res) => {
        try {
            ...
            const deletedUser = await User.findOneAndDelete({ username: deletedEmployee.name });
